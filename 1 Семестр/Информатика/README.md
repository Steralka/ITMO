...
Посещение | 0 | 10 |
Экзамен | 7 | 20 |
* Лабораторная работа N - Смотреть таблицу ```Вес баллов по каждой точке```. При защите лабораторных работ вам выставляют % защиты, после этого переводиться в баллы по формуле: ```C = max/100 * % защиты``` и ```(C >= min)```.
